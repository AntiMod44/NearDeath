	...
	class Enemy {
		...
		draw() {
			...
			be.zIndex = 1;
			be.fillStyle = "black";
			be.roundRect(this.x - this.maxRadius - 4, this.y - this.radius - 26, 2 * this.maxRadius + 8, 13 + 8, 8);
			be.fill();
			be.closePath();
			be.beginPath();
			be.fillStyle = "#8ecc51";
			be.roundRect(this.x - this.maxRadius, this.y - this.radius - 26 + 4, 2 * this.radius, 13, 7);
			be.fill();
			be.closePath();
		}
	...
	function animate() {
		...
		if(document.getElementById("eName").value == "") {
			document.getElementById("test2").innerHTML = "annonimous";
		} else {
			document.getElementById("test2").innerHTML = name;
		}
		document.getElementById("test2").style.top = playerY + 20 + "px";
		document.getElementById("test2").style.left = playerX + "px";
		player.draw();
		update();
		...
		enemies.forEach((enemy, index) => {
			...
			projectiles.forEach((projectile, projectileIndex) => {
				...
				if(dist - enemy.radius - projectile.radius < 1) {
					...
					if(enemy.radius - 10 > 4 && Normal == true) {
					...
					} else if(enemy.radius - 10 > 4 && Hard == true) {
						...
						projectiles.splice(projectileIndex, 1);
					} else if(enemy.radius - 10 > 4 && Insane == true) {
