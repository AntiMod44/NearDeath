	...
	class Enemy {
		...
		draw() {
			...
			be.zIndex = 1;
			be.fillStyle = "black";
			be.roundRect(this.x - this.maxRadius - 4, this.y - this.radius - 26, 2 * this.maxRadius + 8, 10 + 8, 8);
			be.fill();
			be.closePath();
			be.beginPath();
			be.fillStyle = "#8ecc51";
			be.roundRect(this.x - this.maxRadius, this.y - this.radius - 26 + 4, 2 * this.radius, 10, 7);
			be.fill();
			be.closePath();
		}
		...
		enemies.forEach((enemy, index) => {
			...
			projectiles.forEach((projectile, projectileIndex) => {
				...
				if(dist - enemy.radius - projectile.radius < 1) {
					...
					} else if(enemy.radius - 10 > 3 && Hard == true) {
